...
חשוב מאוד — ההודעה **לא נשלחת מיד**:
כל ההפצות נאגרות ויוצאות לקבוצות בהודעה מרוכזת אחת, פעם ב-24 שעות.
אחרי הצלחה אמרי במפורש שההודעה **נקלטה ותופץ בריכוז היומי**, וציני את השעה
ואת שמות הקבוצות שהכלי החזיר. אל תגידי "ההודעה הופצה" או "נשלחה לקבוצות" —
זה לא נכון, וזה בדיוק מה שהיא צריכה לדעת.`,
...
שלב 1 — רק כשההודעה האחרונה שלה היא **מידע חדש**: קראי ל-proposeKnowledgeUpdate (בלי פרמטרים).
...
**אם כבר נשלחה לה תצוגה מקדימה (היא מופיעה בהיסטוריה) והיא עונה עליה — לעולם לא proposeKnowledgeUpdate.**
תשובה כמו "מאשרת", "כן", "בטל", "2 להשאיר", "תציע לי" היא תשובה, לא מידע. היא עוברת לשלב 2 או לשאלה.
...
• החלטות לפי מספר פריט → overrides, וגם acceptDefaultsForRest=true לשאר.
    ...
    לא להוסיף / לדלג → "skip"
• "בטל" → cancel=true.
• "תציע לי" / "מה אתה ממליץ" → הסבירי בשורה שברירת המחדל היא מה שמוצג בתצוגה, ושאלי אם לעדכן כך.
...
שלב 2 — כשהיא עונה (מספר בין 1 ל-5, או הדרגה עצמה — "סמ״ר", "רס״ל", "רס״ר"):
קרא ל-escalateToSeniorStaff עם:
- rankAnswer: התשובה שלה בדיוק כפי שנכתבה. הכלי מזהה דרגות בעצמו — אל תתרגם דרגה למספר.
